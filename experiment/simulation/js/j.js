  ...
  if (e1 == undefined)    // Correction implemented on January 8th, 2012; now V1.1
	return;
        ...
        if (p_flag == 1) {
                los[e2]=e1+1;
                click_count++;
                document.getElementsByTagName("TR")[e1].getElementsByTagName("TD")[e2].style.backgroundImage="url(images/queen.jpeg)";
                if (click_count == N) {
...
function setzeDame () {
  ...
  if (x > 0) {
        var zeile=x;
        var spalte=cc;
        document.getElementsByTagName("TR")[zeile-1].getElementsByTagName("TD")[spalte].style.backgroundImage="url(images/queen.jpeg)";
  }
